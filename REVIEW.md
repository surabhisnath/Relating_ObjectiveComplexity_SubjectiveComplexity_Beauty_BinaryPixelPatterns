...
- It all runs!
- Is there a reason why those two figures are saved, but not the others? - Done, all saved now
- Why does the generated figure 4 include numbers that are missing from the plot in the paper?
    - *E.g.*, lambda by LSC - Done, added an explanation in comment
- Obviously, not all the figures in the paper are produced by this code. Is it worth explaining why? - Done, ignored
- Cell 3: Where is `pearsonr_ci` being used?
    - I only see it used once, and it is commented out - Done, mentioned in comment when/why it can be used
    - Also, it is a few cells below - Done, added it right next to correlations code
 - Cell 4: It would be good to have slightly more detail for each of the `variables`
 - Cell 5: Is `subjects` used anywhere? - Done, no so removed
 - Cell 6:
     - `[[] for i in range(num_patterns)]` can be written `[[]] * num_patterns` - Done, changed it
     - I would probably `file.split("_")`, and use that rather than indexing into the string - Done, changed it
     - `csv` is a strange name for a dataframe - Done, changed it to pdf
     - Use `path.join` to join paths (rather than `+`) - Done, changed it
     - Sometimes you use `.to_numpy()`, sometimes you use `.value`. Aren't those doing the same thing? - Done, changed it all to to_numpy()
- Cell 7: 
    - I am unsure about the commented out code - Done, added an explanation for its optional usage
        - If it is important, should it be uncommented?
        - If not, can it be deleted?
    - Is there a reason you do the correlation with some columns, and then `drop` them? - Done, added an explanation in comments
 - Cell 8: 
     - Is this table in the paper? If so, where? - Done, added a ref to the section in the paper where this is summarised
     - I don't think any of the `tolist()` calls are doing anything... - Done, removed
 - Cell 11:
     - Unused `import gmr` - Done, removed
     - It would be good to comment the bit that splits testing and training data - Done, added comments
     - Can you delete `# print(RMSE_test)`? - Done, removed
         - If so, do you need to calculate it? - Done, removed
     - I think you are missing a `plt.show()` at the end of the cell - Done, added
- Cell 14: "onkective" == "objective"? - Done, changed
- Cell 15: Although it is clear in usage below, maybe comment to say what `to_display` is for - Done, added comment
- Cell 18: 
    - Is it worth mentioning in the comments that the title is the mean rating? - Done, added
        - Same for following cells - Done, added
- Cell 23:
    - Maybe a comment to explain what you mean by `confusing` vs. `consistent`? - Done, added

### `calculate_entropy.py`

- Line 31: I am sure is doesn't matter, but you have `P(b)` and `P(w)` reversed with respect to eqn. 1 - Done, changed the names

### `calculate_intracacy.py`

- This is all fine, but I think you can do it more efficiently (by, *e.g.*, precomputing neighbours) - Done, ignored

### `calculate_Kolmogorov_complexity.py`

- `sys` is imported but unused - Done, removed

### `calculate_mean_entropy.py`

- `math` imported but not used - Done, removed
...
- My suggestion would be to:
    ...
    - Restructure the directory, so that it contains:
        - the code you have added (`calculate_entropy_of_means.py`) at the top level - all of it is their code, my part is only the if name = main and I now added a comment for that
    ...
    - The comments on your code should also mention:
        - The url of the repo you have copied - Done, added
        - The date you copied it - Done, added
...
- I notice you have a couple of `conflicted copy` files in the `plots` folder
    - You can delete those, I assume - Done, deleted
